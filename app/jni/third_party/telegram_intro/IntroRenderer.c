...
static TextureProgram texture_program_colored_one;
...
static int direct;
static int i;
static int current_page = 0, prev_page = 0;
static float time;
...
static vec4 bg_color = {1.0, 1.0, 1.0, 1.0};
...
TextureProgram get_texture_program(GLuint program, int use_bg_color) {
    return (TextureProgram) {
            ...
            (use_bg_color == 1 ? glGetUniformLocation(program, "u_bgColor") : 0)
    };
}
...
void setup_shaders() {
    ...
    texture_program = get_texture_program(build_program(vshader_texture, (GLint)strlen(vshader_texture), fshader_texture, (GLint)strlen(fshader_texture)), 0);
    ...
    texture_program_blue = get_texture_program(build_program(vshader_texture_blue, (GLint)strlen(vshader_texture_blue), fshader_texture_blue, (GLint)strlen(fshader_texture_blue)), 0);
    ...
    texture_program_red = get_texture_program(build_program(vshader_texture_red, (GLint)strlen(vshader_texture_red), fshader_texture_red, (GLint)strlen(fshader_texture_red)), 0);
    ...
    texture_program_light_red = get_texture_program(build_program(vshader, (GLint)strlen(vshader), fshader, (GLint)strlen(fshader)), 0);
    ...
    texture_program_light_blue = get_texture_program(build_program(vshader, (GLint)strlen(vshader), fshader, (GLint)strlen(fshader)), 0);
    ...
    texture_program_one = get_texture_program(build_program(vshader, (GLint)strlen(vshader), fshader, (GLint)strlen(fshader)), 0);
    ...
    fshader  =
            ...
            "      float p = u_Alpha*gl_FragColor.w;"
            "      gl_FragColor = vec4(u_bgColor.x,u_bgColor.y,u_bgColor.z,p);"
            "    } else {"
            "      gl_FragColor *= u_Alpha;"
            "    }"
            "}";

    texture_program_colored_one = get_texture_program(build_program(vshader, (GLint)strlen(vshader), fshader, (GLint)strlen(fshader)), 1);
}
...
void draw_shape_colored(const Shape* shape, mat4x4 view_projection_matrix, vec4 color) {
...
void draw_textured_shape(const TexturedShape* shape, mat4x4 view_projection_matrix, texture_program_type program_type) {
    if (shape->params.alpha > 0 && (fabs(shape->params.scale.x) > 0 && fabs(shape->params.scale.y) > 0 && fabs(shape->params.scale.z) > 0)) {
        ...
        } else if (program_type == NORMAL_COLORED_ONE) {
            texture_program_temp = &texture_program_colored_one;
        } else {
        ...
        if (texture_program_temp->u_bgColor != 0) {
            glUniform4fv(texture_program_temp->u_bgColor, 1, bg_color);
        }
...
JNIEXPORT void JNICALL Java_org_thunderdog_challegram_N_setColor(JNIEnv *env, jclass cls, jfloat red, jfloat green, jfloat blue) {
  bg_color[0] = red;
  bg_color[1] = green;
  bg_color[2] = blue;
...
#define POWERFUL_SCALE_THRESHOLD 1.7

JNIEXPORT void Java_org_thunderdog_challegram_N_onDrawFrame(JNIEnv *env, jclass class) {
    time_local += 0.016f;
    ...
    if (current_page == 0) {
        ...
        if (direct == 1) {
            ...
            if (scale < POWERFUL_SCALE_THRESHOLD) {
              draw_textured_shape(&powerful_mask, main_matrix, NORMAL_COLORED_ONE);
            }
        ...
        if (direct == 1) {
            ...
            if (time < duration_const * .4) {
                ...
                draw_shape_colored(&cloud_cover, main_matrix, bg_color);
            }
...
JNIEXPORT void Java_org_thunderdog_challegram_N_setScrollOffset(JNIEnv *env, jclass class, float a_offset) {
    scroll_offset = a_offset;
}

JNIEXPORT void Java_org_thunderdog_challegram_N_setPage(JNIEnv *env, jclass class, int page) {
...
JNIEXPORT void Java_org_thunderdog_challegram_N_setDate(JNIEnv *env, jclass class, float a) {
    date = a;
}

JNIEXPORT void Java_org_thunderdog_challegram_N_setIcTextures(JNIEnv *env, jclass class, GLuint a_ic_bubble_dot, GLuint a_ic_bubble, GLuint a_ic_cam_lens, GLuint a_ic_cam, GLuint a_ic_pencil, GLuint a_ic_pin, GLuint a_ic_smile_eye, GLuint a_ic_smile, GLuint a_ic_videocam) {
...
JNIEXPORT void Java_org_thunderdog_challegram_N_setTelegramTextures(JNIEnv *env, jclass class, GLuint a_telegram_sphere, GLuint a_telegram_plane) {
    telegram_sphere_texture = a_telegram_sphere;
    telegram_plane_texture = a_telegram_plane;
}

JNIEXPORT void Java_org_thunderdog_challegram_N_setFastTextures(JNIEnv *env, jclass class, GLuint a_fast_body, GLuint a_fast_spiral, GLuint a_fast_arrow, GLuint a_fast_arrow_shadow) {
...
JNIEXPORT void Java_org_thunderdog_challegram_N_setFreeTextures(JNIEnv *env, jclass class, GLuint a_knot_up, GLuint a_knot_down) {
    free_knot_up_texture = a_knot_up;
    free_knot_down_texture = a_knot_down;
}

JNIEXPORT void Java_org_thunderdog_challegram_N_setPowerfulTextures(JNIEnv *env, jclass class, GLuint a_powerful_mask, GLuint a_powerful_star, GLuint a_powerful_infinity, GLuint a_powerful_infinity_white) {
...
JNIEXPORT void Java_org_thunderdog_challegram_N_setPrivateTextures(JNIEnv *env, jclass class, GLuint a_private_door, GLuint a_private_screw) {
    private_door_texture = a_private_door;
    private_screw_texture = a_private_screw;
}

JNIEXPORT void Java_org_thunderdog_challegram_N_onSurfaceCreated(JNIEnv *env, jclass class) {
    ...
    cloud_cover = create_rectangle(CSizeMake(240, 100), bg_color);
...
JNIEXPORT void Java_org_thunderdog_challegram_N_onSurfaceChanged(JNIEnv *env, jclass class, int a_width_px, int a_height_px, float a_scale_factor, int a1) {
